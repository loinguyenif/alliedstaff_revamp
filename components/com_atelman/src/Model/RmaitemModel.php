 ...
 * @since  1.0.0
 */
class RmaitemModel extends ItemModel
{
	public $_item;
	...
	public function getItemIdByAlias($alias)
	{
		...
		if (method_exists($this, 'getAliasFieldNameByView')) {
			$aliasKey   = $this->getAliasFieldNameByView('rmaitem');
		}
	...
	{
		// Get the id.
		$id = (!empty($id)) ? $id : (int) $this->getState('rmaitem.id');
	...
	public function save($post)
	{
		...
		$arr_product 			= 	$post['product_id'];
		$arr_serial_no			= 	$post['serial_no'];
		$arr_description		= 	$post['description'];
		$arr_invoice_no			= 	$post['invoice_no'];
		$arr_remarks			= 	$post['remarks'];
		...
		for ($i = 0; $i < $total; $i++) :
			...
			$serial_no	= $arr_serial_no[$i];

			$where = array();

			$where[]	=	" product_id = " . $db->Quote($product_id, false);
			$where[]	=	" serial_no = " . $db->Quote($serial_no, false);

			$where 		= (count($where) ? ' WHERE ' . implode(' AND ', $where) : '');

			$query = " SELECT id,expired_date FROM #__at_warranty_items " . $where;
			$db->setQuery($query);
			$witem = $db->loadObject();

			// warranty is still in-valid or out-valid
			$wstatus = '';
			$rmacode = '';
			...
			$atelrmaitem->warranty_item_id		= 	$witem->id;
			$atelrmaitem->warranty_status		=	$wstatus;
			$atelrmaitem->description			=	$arr_description[$i];
			$atelrmaitem->invoice_no			=	$arr_invoice_no[$i];
			$atelrmaitem->remarks			=	$arr_remarks;
