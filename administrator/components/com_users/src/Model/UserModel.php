...
class UserModel extends AdminModel implements UserFactoryAwareInterface
{
    ...
    public function save($data)
    {
        ...
        //ifoundries
        $groupsName = Access::getGroupTitle($data['groups'][0]); // Trả về mảng id => tên
        $user->usertype = $groupsName;
        $user->gid = $data['groups'][0];
    ...
    public function sendMailRequest($post)
    {
        ...
        foreach ($super_administrator as $r) {
            if ($r->sendEmail) {
                array_push($bcc_recipients, $r->email);
            }
        }
        ...
        $subject = $app->get('sitename') . ' - User Account Request';

        $body    .= '<div style="margin:10px 0;">Account Request by ' . $user->name . ' ( ' . $user->username . ' ) :</div>';
        ...
        $body    .= '<tr><td>Email Address</td><td>' . $post['email'] . '</td></tr>';
        $body    .= '<tr><td>Job Title</td><td>' . $post['job_title'] . '</td></tr>';
        $body    .= '<tr><td>Company Name</td><td>' . $post['company_name'] . '</td></tr>';
        $body    .= '<tr><td>Website URL</td><td>' . $post['website'] . '</td></tr>';
        $body    .= '<tr><td>Address</td><td>' . nl2br($post['address']) . '</td></tr>';
        $body    .= '<tr><td>City</td><td>' . $post['city'] . '</td></tr>';
        $body    .= '<tr><td>State</td><td>' . $post['state'] . '</td></tr>';
        $body    .= '<tr><td>Postal Code</td><td>' . $post['postalcode'] . '</td></tr>';
        $body    .= '<tr><td>Country</td><td>' . $post['country'] . '</td></tr>';
        $body    .= '<tr><td>Phone</td><td>' . $post['phone'] . '</td></tr>';
        $body    .= '</table>';
        ...
        //$mail->addRecipient(array('ajs6683@yahoo.com'));
