 ...
 * @since    0.1b
 */
class com_atelmanInstallerScript extends InstallerScript
{
	/**
	...
	private function runUpdateSql($parent)
	{
	    $installation_folder = $parent->getParent()->getPath('source');
		...
		if (method_exists($parent, 'getManifest')) {
			$versionstring = $parent->getManifest()->version;
		}
		else {
			$versionstring = $parent->get('manifest')->version;
		}
		
		$versionparts 	= explode('CVS:', $versionstring);
		
		$version 		= trim($versionparts[1]);
		
		$file 			= $installation_folder . '/administrator/sql/updates/'.$version.'.sql';
