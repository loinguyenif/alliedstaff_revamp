...
class CountryModel extends AdminModel
{
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_atelman.country';
	 ...
	 * @since   1.0.0
	 */
	public function getTable($type = 'Country', $prefix = 'Administrator', $config = array())
	{
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_atelman.edit.country.data', array());
	...
	/**
	 * Method to duplicate an Country
	 *
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__at_countries');
				$max             = $db->loadResult();
	...
	public function save($post)
	{
		...
		$atelcountry = $this->getTable('Country');

		if (!$atelcountry->bind($post)) {
			return false;
		}

		$atelcountry->id = $post['id'];

		if ($atelcountry->store()) {
			...
			if ($post['id']) {
				...
				$log->action_remarks = "Update Country :  -> " . $post['country'];
				$log->id =	$post['id'];
			} else {
				$log->action_type = 'ADD';
				$log->action_remarks = "Add Country : " . $post['country'];
				$log->id =	$atelcountry->id;
			}
