...
class LogsModel extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__at_logs` AS a');

		// Join over the user
		$query->select('u.name');
		$query->join('LEFT', '#__users AS `u` ON `u`.id = a.`action_by`');
		...
		if (!empty($search)) {
			if (stripos($search, 'id:') === 0) {
				...
				$searchConditions[] = 'a.section LIKE ' . $search;
				$searchConditions[] = 'a.action_type LIKE ' . $search;
				$searchConditions[] = 'u.name LIKE ' . $search;
