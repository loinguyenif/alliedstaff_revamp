...
class LogModel extends AdminModel
{
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_atelman.edit.log.data', array());
	...
	public function getItem($pk = null)
	{
		...
		$query = $db->getQuery(true)
			->select('l.*, u.name')
			->from($db->quoteName('#__at_logs', 'l'))
			->join('LEFT', $db->quoteName('#__users', 'u') . ' ON ' . $db->quoteName('u.id') . ' = ' . $db->quoteName('l.action_by'))
			->where($db->quoteName('l.id') . ' = ' . (int) $pk);
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__at_logs');
				$max             = $db->loadResult();
